...
export function SwitchArrows(
    props: Omit<IProps, "size"> & { className?: string; style?: React.CSSProperties; shouldNotAdjust?: boolean },
) {
    const { className, onClick, squareDimension, shouldNotAdjust, style, team } = props;
    ...
    return (
        <div className={className} style={style}>
            <svg
                height={shouldNotAdjust ? squareDimension : squareDimension * 0.25}
                width={shouldNotAdjust ? squareDimension : squareDimension * 0.45}
                style={style}
                onClick={onClick}
            >
                <g
                    ...
                    xmlns="http://www.w3.org/2000/svg"
                    transform={
                        shouldNotAdjust ? "translate(10, 125) scale(0.13, -0.13)" : "translate(0,75) scale(0.1,-0.1)"
                    }
                >
